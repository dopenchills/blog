...
リポジトリを分けてしまったがゆえ、あるリポジトリのコードをパッケージ化して社内レジストリに公開し、それを別リポジトリで消費するようなことをしている場合もある。こうすれば互換性の問題は起きない。が、パッケージにして公開してバージョンを管理するような営み自体が、やってみると結構な手間だ。
...
フロントエンドのモノレポなら、依存のアップグレードは一箇所だけだ。

バックエンドでもインフラでも、同じことが言える。
