...
ここでは細かく触れないが、その他のテーブルにもレコードが追加される。

例外が出たらexceptionsテーブルにレコードが追加される。

サーバーに通信したらdependenciesテーブルにレコードが追加される。

カスタムイベントを定義して、customEventsテーブルで表示させることもできる。

イメージを掴むのに以下のページが参考になった。
