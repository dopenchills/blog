...
以下、複数リポジトリを「ポリレポ」と呼び、単一レポジトリにまとめることを「モノレポ」と呼ぶ。
...
ポリレポでは、依存のアップグレードはリポジトリごとに行う必要がある。

モノレポなら、依存のアップグレードは一箇所だけで済む。

例えばNode.jsのバージョン指定にはmiseの設定ファイルを一つ置けばいい。フロントエンドのパッケージアップグレードにはpnpmのcatalog機能で一括管理できる。

バックエンドでもインフラでも同様のことが言える。モノレポなら依存管理の仕組みが単純化できる。
...
以下のように機能ごとに適切にディレクトリを区切っていれば、ポリレポで作業するのと散らかり具合もコンフリクトの頻度も変わらないはずだ。
...
もし散らかったりコンフリクトが多発するのであれば、たぶんポリレポでも同じことが起きる。
...
モノレポのメリットとして、コーディングエージェントとの相性が良いというのが挙げられる。

コーディングエージェントにバックエンド、フロントエンド、インフラの構造を簡単に共有できるため、調査も実装も楽なのだ。
...
ポリレポが必要なのは、チーム開発において、リポジトリの可視性の管理が必要になるときだ。
...
まずはモノレポから始める。今理由なくポリレポを使っている場合は`git subtree`でモノレポにする。
